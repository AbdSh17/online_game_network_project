...
# general Vars
question, answer, number_of_answers, players_count, number_of_questions, number_of_correct_answers, question_votes, number_of_votes = None, None, 0, 0, 0, 0, {"1": 0, "2": 0, "3": 0, "4": 0}, 0
...
def join_a_game(server_socket, client_address, data):
    ...
    client = client_states[client_address]
    global question, answer, number_of_answers, players_count, number_of_questions, number_of_correct_answers, question_votes, number_of_votes
    if data == "back":
        client["step"] = "option"
    ...
    if players_count < 2:
        if client["step"] == "game_waiting":
            ...
            voting_message = "Please select one of these 4 genres\n1- History\n2- Anime\n3- Math\n4- Geography"
            for clients_addresses in client_states.keys():
                if client_states[clients_addresses]["step"] == "game_waiting" or client_states[clients_addresses]["step"] == "in_game":
                    client_states[clients_addresses]["step"] = "voting"
            ...
            time.sleep(15)
            for clients_addresses in client_states.keys():
                if client_states[clients_addresses]["step"] == "game_waiting" or client_states[clients_addresses]["step"] == "voting":
                    server_socket.sendto(voting_message.encode(), clients_addresses)
            return None

    if client["step"] == "voting":
        if 1 <= int(data) <= 4:
            question_votes[f"{str(data)}"] += 1

    print("Entered=csavdsavdvdv===============================")
    if sum(question_votes.values()) == players_count and client["step"] == "voting":
        print("Entered=csavdsavdvdv===============================")
        number = 10 * int(max(question_votes, key=question_votes.get))
        question_number = random.randint((number - 9), number)
        question, answer = questions_set_all(question_number)
        print(f"question: {question}  Answer: {answer}  number: {question_number}")
        for clients_addresses in client_states.keys():
            if client_states[clients_addresses]["step"] == "game_waiting" or client_states[clients_addresses]["step"] == "voting":
    ...
    if number_of_answers == players_count:
        ...
        max_points, winner_client = -1, None
        for clients_addresses in client_states.keys():
            if client_states[clients_addresses]["step"] == "game_waiting" or client_states[clients_addresses]["step"] == "in_game":
                standings += f"Player Name:  {client_states[clients_addresses]["id"]} -- Player Points: {client_states[clients_addresses]["points"]} \n"
                if max_points < client_states[clients_addresses]["points"]:
                    max_points = client_states[clients_addresses]["points"]
